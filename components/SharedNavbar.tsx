    ...
    <>
      <nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out
          ${scrolled
            ? 'bg-white/90 backdrop-blur-2xl border-b border-blue-200/70 shadow-sm'
            : 'bg-white/70 backdrop-blur-xl border-b border-blue-200/40'
          }`}
      >
        <div className="max-w-5xl mx-auto px-5 sm:px-8 h-14 flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-0.5">
              {links.map(l => (
                <Link
                  ...
                  className="px-3 py-1.5 text-[13px] text-blue-600 hover:text-blue-900 rounded-md hover:bg-blue-100/60 transition-all duration-150"
                >
                  {l.label}
                </Link>
              ))}
          ...
          <button
            onClick={() => setOpen(v => !v)}
            className="md:hidden flex flex-col gap-1.5 p-2 rounded-md text-blue-500 hover:text-blue-800 transition-colors"
      ...
      >
        ...
        {/* Drawer */}
        <div
          className={`absolute top-0 left-0 right-0 bg-white/98 backdrop-blur-2xl border-b border-blue-200 transition-all duration-300 ease-out ${open ? 'translate-y-0' : '-translate-y-full'}`}
        >
          <div className="px-5 pt-5 pb-4 flex items-center justify-between border-b border-blue-100">
            <Link href="/" onClick={() => setOpen(false)} className="flex items-center gap-2">
              <span className="text-lg">{brand.icon}</span>
              <span className="font-semibold text-blue-900 text-sm">{brand.name}</span>
            </Link>
            <button onClick={() => setOpen(false)} className="p-1.5 text-blue-400 hover:text-blue-700 transition-colors">
          ...
          <div className="px-5 py-4 flex flex-col gap-1">
            {links.map(l => (
              <Link
                ...
                href={l.href}
                onClick={() => setOpen(false)}
                className="px-2 py-2.5 text-sm text-blue-600 hover:text-blue-900 rounded-lg hover:bg-blue-50 transition-all"
              >
                {l.label}
              </Link>
            ))}
